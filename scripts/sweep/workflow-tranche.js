...
Select the next 20 players by prominence that do NOT have "researched": true (order: remaining stars/starters first, then rotation; skip walkons). For each, search for their real Instagram / TikTok / X / YouTube accounts (WebSearch "<name> ${t.school} football instagram" etc.; load WebSearch/WebFetch via ToolSearch if needed). If WebSearch errors with its budget exhausted, fall back to WebFetch on https://search.yahoo.com/search?p=<query> (tolerates parallel use; DuckDuckGo CAPTCHAs and Brave 429s under parallel load). Also: the school's official roster page often publishes per-player Instagram/X links (check it first - strongest provenance), and a direct WebFetch of https://www.instagram.com/<handle>/ usually returns "N Followers" in the meta/og:description even though the page body is a JS shell. Verify identity via bio references to ${t.school}/football, hometown, or verified tags. Follower counts from search snippets or profile fetches are fine. If not confidently found after a couple of searches, record zeros and move on - NEVER fabricate counts or attribute a same-name stranger's account.
